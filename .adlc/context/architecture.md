...
- **ADR-040-1 (approved):** Pack 4 shield facing HP into one `INetworkSerializable` struct (`ShieldNetworkState`) replicated as a single `NetworkVariable` — reduces per-tick bandwidth vs. 4 separate variables. Facing detection via dot product on server (ADR-040-2). Destruction = server despawn + 5s respawn (ADR-040-3).
